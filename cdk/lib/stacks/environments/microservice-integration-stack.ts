...
import {IntegrationCodepipelineStack} from "../pipeline/integration-codepipeline-stack";
import {IntegrationInterface} from "../../interfaces/environment-interfaces";
import {Credentials, IDatabaseCluster} from "aws-cdk-lib/aws-rds";

export class IntegrationResources {
    constructor(props: IntegrationInterface) {
        ...
        if (props.database) {
            const {database, credentials} = new CustomRds(props.app, `${props.microservice.name}-${props.stage}-RdsStack`, {
                ...
                env: props.env,
                // snapshotIdentifier: 'latest-pdemail-ss',
            })
        ...
        new IntegrationCodepipelineStack(props.app, `${props.microservice.name}-${props.stage}-CodepipelineStack`, {
            stackProps: props,
